...
def editItem(item_id):
    ...
    if request.method == 'POST':
        ...
        item_to_edit.created_date = datetime.utcnow()
        session.add(item_to_edit)
        session.commit()
        return redirect(url_for('single_catagory', catalog_id=item_to_edit.catalog_id, item_id=item_id))
    else:
        categories = session.query(Catalog).all()
...
def deleteItem(item_id):
    ...
    if request.method == 'POST':
        ...
        directory_catalog = {}
        directory_catalog['link'] = url_for('items_by_catalog', catalog_id=item_to_delete.catalog.id)
        directory_catalog['name'] = item_to_delete.catalog.name
        directory.append(directory_catalog)

        directory_item = {}
        directory_item['link'] = url_for('single_catagory', catalog_id=item_to_delete.catalog.id, item_id=item_to_delete.id)
        directory_item['name'] = item_to_delete.title
        directory.append(directory_item)
